...
FROM_IMG=172.19.20.50/dev-support/build-env:gcc-base-1.0
IMG_NAME=gcc-build
...
docker build -t "${IMG_NAME}-${USER_ID}" - <<UserSpecificDocker
FROM ${FROM_IMG}
RUN rm -f /var/log/faillog /var/log/lastlog
RUN groupadd --non-unique -g ${GROUP_ID} ${USER_NAME}
RUN useradd -g ${GROUP_ID} -u ${USER_ID} -k /root -m ${USER_NAME} -d "${DOCKER_HOME_DIR}"
RUN echo "${USER_NAME} ALL=NOPASSWD: ALL" > "/etc/sudoers.d/${IMG_NAME}-${USER_ID}"
...
docker run --rm=true $DOCKER_INTERACTIVE_RUN \
  -v "${PWD}:${DOCKER_HOME_DIR}/workspace${V_OPTS:-}" \
  -v "${HOME}/.gnupg:${DOCKER_HOME_DIR}/.gnupg${V_OPTS:-}" \
  -u "${USER_ID}" \
  -w "${DOCKER_HOME_DIR}/workspace" \
  --name "${IMG_NAME}-${USER_ID}" \
  "${IMG_NAME}-${USER_ID}" "$@"
